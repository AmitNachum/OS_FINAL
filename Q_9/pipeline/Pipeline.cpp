...
Pipeline::Pipeline(SendFunc sender)
    : q_in(Q_CAP_IN),
      ...
      q_out(Q_CAP_OUT)
{
    aggregator = std::make_unique<AO_Aggregator>(q_results, q_out);
    responder  = std::make_unique<AO_Responder>(q_out, std::move(sender));
    fanout     = std::make_unique<AO_Fanout>(q_in, *aggregator, q_mst, q_scc, q_ham, q_flow);
}
