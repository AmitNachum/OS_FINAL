...
int main() {
    try {
        ...
        while (true) {
            ...
            // Send init early (same as stage 8 style)
            ...
            if (mode == "random") {
                ...
                while (true) {
                    int u, v, w;
                    std::cin >> u >> v >> w;
                    if (!std::cin) { // recover from bad input
                        std::cin.clear();
            ...
            int maxflow_src = 0, maxflow_sink = (n > 0 ? n - 1 : 0);
            std::cout << "Max-Flow source vertex: ";
            if (!(std::cin >> maxflow_src)) {
                std::cin.clear();
                std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
                maxflow_src = 0;
            }
            std::cout << "Max-Flow sink vertex: ";
            if (!(std::cin >> maxflow_sink)) {
                std::cin.clear();
                std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
                maxflow_sink = (n > 0 ? n - 1 : 0);
            }
